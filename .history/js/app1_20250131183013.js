    ...
    fetch("../js/articulos.json")
        ...
        .then(data => {
            ...
            data.forEach(articulo => {
                ...
                // Si existe una imagen, reemplazamos la etiqueta {{imagen}} en el contenido
                if (articulo.imagen) {
                    cardContent = cardContent.replace("{{imagen}}", `<img src="${articulo.imagen}" alt="Imagen del artículo" class="article-image"/>`);
                } else {
                    cardContent = cardContent.replace("{{imagen}}", "");  // Si no hay imagen, eliminar el marcador
                }
