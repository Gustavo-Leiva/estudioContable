document.addEventListener("DOMContentLoaded", function () {
        ...
        data.forEach((articulo) => {
          ...
          articulo.contenido.forEach((bloque) => {
            if (bloque.tipo === "texto") {
              ...
              contenidoHTML += `<ul>${generarLista(bloque.valor)}</ul>`;
            }
          });
    ...
    // Función para generar las listas, incluyendo las anidadas
    function generarLista(lista) {
      let htmlLista = "";
      lista.forEach((item) => {
        if (Array.isArray(item)) {
          htmlLista += "<ul>";
          htmlLista += generarLista(item); // Llamada recursiva para listas anidadas
          htmlLista += "</ul>";
        } else {
